...
<body>
	...
	if (rs1.next()) {
		...
		if (rs2.next()) {
			...
			String role = rs2.getString("role"); // Get the role of the user
			session.setAttribute("user", userid); // Set the username in the session
			session.setAttribute("role", role); // Set the role in the session

			// Redirect to the success page or customer portal
